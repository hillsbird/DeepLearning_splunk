...
class MLPClassifier(ClassifierMixin, BaseAlgo):
    def __init__(self, options):
        ...
        if 'activation' in out_params and out_params.get('activation') not in supported_activations:
            raise RuntimeError('Invalid value for activation: "{}" must be one of {}.'.format(out_params['activation'], str(supported_activations)))

        if 'momentum' in out_params and not (0 <= out_params.get('momentum') <= 1):
            raise RuntimeError('Invalid value for momentum: "{}" must be >= 0 and <= 1.'.format(out_params['momentum']))
